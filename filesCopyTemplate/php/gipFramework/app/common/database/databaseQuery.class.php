...
class databaseQuery extends databaseConnectionPool
{
	...
	*/
	function executeQuery()
	{
		...
		// Count Rows and set it in TotalRows field
		$this->countResultSet();
		...
		if ($result==false)
		{
			...
			$thisError->setUserErrorMessage("An error occured while executing query from database. ");
			$thisError->setProgramErrorMessage("SQL Query : ".$this->getSqlQuery()."\n\nError from db : ".$this->dbConn->ErrorMsg());
	...
	function executeDirectQuery($query)
	{
		...
		if ($result==false)
		{
			$thisError = new errorHandler();
			$thisError->setUserErrorMessage("An error occured while executing query from database. ");
			$thisError->setProgramErrorMessage("SQL : ".$this->getSqlQuery()." ... Error : ".$this->dbConn->ErrorMsg());
			$thisError->setErrorPage($_SERVER['PHP_SELF']);
			$thisError->handleError();
